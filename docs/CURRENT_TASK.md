...
- Stato: **completo e verificato in browser** (ADR-2026-063, ADR-2026-064, ADR-2026-065, ADR-2026-066). 99 eventi G2 reali + 1 evento Schmid + 19 bonus-logo mancanti scritti sul ledger; profilo comportamentale per squadra (`market_model.team_preference_profiles`) da 616 righe di preferenza curate; pagina Buste G3 con store/feasibility dedicati (spesa peggiore = somma, non massimo). 463 test totali passano.
...
- Gap noti, non bloccanti: Lista 1 (portieri, formato a nomi di club non ancora gestito dal resolver) non ingerita nello storico preferenze; ~20% delle righe delle altre 6 liste escluse per nomi ambigui/non risolti; `team_preference_profiles` non ancora collegato a una pagina UI né a `bid_recommendation.py` (resta un modulo dati pronto per un consumo futuro esplicito, per non introdurre un correttivo di prezzo non richiesto).
- Prossima azione: nessuna bloccante. Possibili estensioni future su richiesta: superficie UI per `team_preference_profiles`; ingest di Lista 1 (portieri); collegare il profilo comportamentale a `bid_recommendation.py`.
