...
| Understat | xG, npxG, xA, tiri, minuti | MVP privato con cache/rate limit; non dipendenza irremovibile | **corretto 2026-08-12**: `robots.txt` di understat.com dichiara `Disallow: /` per ogni user-agent — accesso automatizzato vietato esplicitamente, non solo "da usare con cautela". **Escluso**, stesso trattamento di Fantacalcio.it (CLAUDE.md: mai scraping di fonti che lo vietano). |
