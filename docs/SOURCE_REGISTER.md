...
## Altri provider italiani valutati (non integrati)

Su richiesta, ricerca di provider italiani alternativi (2026-08-10): FantaMaster, Fantacalcio-Online (dichiara di aggregare fino a 4 fonti voti) sono aggregatori con lo stesso profilo di rischio ToS di Fantacalcio.it, non fonti indipendenti pulite. `www.legaseriea.it/api/` (endpoint del sito ufficiale Lega Serie A): **verificato 2026-08-10, non è una pagina di documentazione pubblica per sviluppatori (404, nessuna pagina landing)** — indica un endpoint interno del sito, stessa categoria di rischio dell'endpoint privato Fantacalcio.it già escluso in questa sessione. Non consigliato senza una verifica separata di documentazione/ToS ufficiali per l'uso da terzi (se esistono, andrebbero cercati altrove, non assunti dall'URL). football-data.io e apiballs.com non ancora auditati.
