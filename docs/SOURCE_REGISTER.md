...
| Big Balls Sports Data | lineup/eventi/statistiche/xG se disponibile | provider nuovo; audit storico, granularità e contratto | **verificato via documentazione 2026-08-10 (non ancora testato con chiamate reali)**: piano gratuito 1000 req/giorno (2000 con GitHub), nessuna carta di credito, richiede solo signup account per API key. Endpoint confermati: `/v1/stored/matches/:id/lineups` (Starting XI + panchina), `/v1/matches/:id/events` (gol/cartellini/sostituzioni), `/v1/stored/matches/:id/stats` (xG/possesso). Copre Serie A stagione 2025/26 corrente. **Candidato più promettente finora per il gap lineup/minuti** — termini di redistribuzione non ancora verificati (`/legal/terms`), serve account (creazione da parte dell'utente, non automatizzabile). |
