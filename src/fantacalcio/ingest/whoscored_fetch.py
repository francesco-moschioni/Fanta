"""STANDALONE personal-use fetch helper for WhoScored — NOT part of the pipeline.

Engine v2 Stage 7, personal-use override ADR-2026-070 / ADR-2026-079.

WhoScored's Terms of Service forbid automated collection. That is acknowledged
here and NOT worked around at scale: the project owner authorised, for their own
private local fantacalcio only, occasional manual retrieval of
injuries / suspensions / probable-lineup data. To keep the risk contained and
the source replaceable, this script:

* is **standalone** — run as ``python -m fantacalcio.ingest.whoscored_fetch ...``
  or ``python src/fantacalcio/ingest/whoscored_fetch.py ...``. It does its work
...
* is **never imported** by any pipeline module, by ``scripts/run_*.py`` or by CI
  (``tests/test_ingest_whoscored.py`` statically asserts this);
...
If a licensed provider is bought later, only ``ingest/whoscored.py`` changes;
...
DEFAULT_CACHE_DIR = Path("data/raw/_whoscored_fetch_cache")
SOURCE_ID = "whoscored"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Manual, rate-limited WhoScored fetch (personal use)."
    )
    parser.add_argument(
        "paths", nargs="+",
        help="WhoScored paths, e.g. 'Regions/108/Tournaments/5/Seasons/.../Italy-Serie-A'.",
    )
    parser.add_argument("--cache-dir", type=Path, default=DEFAULT_CACHE_DIR)
    parser.add_argument("--filename-prefix", default="whoscored")
    ...
    print("done. Parse the snapshots with fantacalcio.ingest.whoscored (pure, offline).")
    return 0
